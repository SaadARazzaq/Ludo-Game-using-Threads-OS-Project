...
void* playerthread(void* attr)
{
	...
	for(int i=0;i<3;i++){
		...
		cout<<"\n\t\t\033[1;95m ***PLAYER ("<<temp<<") ROLLED DICE...\033[0m";
		cout<<"\n\t\t\033[1;95m ~DICE VALUE IS: \033[0m"<<x[temp-1][i]<<endl;//priinting dice value
		sleep(1);
	...
	for(int j=0;j<3 && x[temp-1][j]>0 ; j++){
		...
		if(flag||x[temp-1][j]==6){
			cout<<"\t\033[1;95mEnter which token you want to move for player: \033[0m"<<temp<<" :";
			cin>>n;
			
			while(n>tokens||n<1){
				cout<<"\t\033[1;91m*Enter value between 1 to 4: \033[0m";
				cin>>n;
			}
			while(((!tempPlayer->tok[n-1].open) && x[temp-1][j]!=6) || (tempPlayer->tok[n-1].win)){
				cout<<"\t\033[1;91mPlease Enter value of opened tokens: \033[0m";
				cin>>n;
			}
			bool winflag=0;
			bool notmoveflag=0;
			...
			{
				cout<<"\t\033[1;91m***ENTERED TOKEN MOVEMENT NOT POSSIBLE***\033[0m"<<endl;
				...
				if(winflag){
					cout<<"\t\033[1;91m***ENTER ANOTHER TOKEN***\033[0m"<<endl;
					cin>>n;
				}
				else
				{
		...
		for(int i=0;i<tokens;i++)
			cout<<"\t\033[1;95m~Position value of token ("<< i+1<< ") of player ("<<temp<<") is:\033[0m "<<tempPlayer->tok[i].value<<endl;
